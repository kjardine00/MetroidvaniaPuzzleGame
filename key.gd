...
func _ready() -> void:
	collection_area.body_entered.connect(func(_body: Player): 
		# print_debug("[Key] key should be collected by : " + _body.name)
		_body.inv.add_key()
		queue_free()
	)
...
func _physics_process(delta: float) -> void:
	...
	# TODO: Revist because I don't want to do it right now but I want the keys to bounce off the floor a small amount
